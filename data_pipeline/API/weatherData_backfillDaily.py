...
`uv run weather_daily_backfill.py -y <year>`
...
def _backfill_daily_weather(years_code:int = args.years) -> int:
...
def main() -> None:
    ...
    # try _backfill_daily_weather, log error if fails
    try:
        EXIT_CODE = _backfill_daily_weather() # let's get our exit code
